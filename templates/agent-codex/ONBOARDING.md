...
> **Environment variables**: `CTX_ROOT`, `CTX_FRAMEWORK_ROOT`, `CTX_ORG`, `CTX_AGENT_NAME`, `CTX_AGENT_DIR`, and `CTX_INSTANCE_ID` are automatically set by the SiriusOS framework. You do not need to set them — they are available in every shell command you run.

> **Runtime:** This is a `codex-app-server` agent. Telegram replies go through `siriusos bus send-telegram <chat_id> '<msg>'`. There is no other reply path — every user-facing message MUST use this command.
...
1. **Introduce yourself** via Telegram (use `siriusos bus send-telegram $CTX_TELEGRAM_CHAT_ID '<msg>'`):
...
9. **Ask for workflows:**
   ...
   - Add it as a persistent cron (survives restarts automatically):
     ...
     ```
   - `siriusos bus add-cron` is the only persistent scheduling path on this runtime. There is no in-session scheduling tool; daemon-managed crons are what survives restarts.
   - If the workflow is complex (multi-step procedure), create a skill file at `plugins/siriusos-agent-skills/skills/<workflow-name>/SKILL.md` with YAML frontmatter and detailed steps, and a corresponding symlink at `~/.codex/skills/${CTX_AGENT_NAME}__<workflow-name>` pointing to that directory.
...
11b. **Ask about migration:**
    ...
    **END YOUR TURN.** If the user says no migration, skip to step 12. If yes, continue below.
    ...
    - Copy any custom skills: for each skill in `<old_dir>/plugins/siriusos-agent-skills/skills/` (or `<old_dir>/.claude/skills/` if migrating from a Claude-Code agent), offer to copy it into `${CTX_AGENT_DIR}/plugins/siriusos-agent-skills/skills/` and create a `~/.codex/skills/${CTX_AGENT_NAME}__<skill_name>` symlink for each
...
13. **Knowledge base setup — ALWAYS DO THIS STEP:**
    ...
    **END YOUR TURN.** Wait for the user's answers.
    ...
    Based on their answers, write memory management rules to `plugins/siriusos-agent-skills/skills/memory/SKILL.md` or a new `plugins/siriusos-agent-skills/skills/memory-management/SKILL.md`:
    ```markdown
...
18. **Confirm with user** via Telegram (`siriusos bus send-telegram`):
...
23. If user wants to set up now, ask sequentially:
    ...
    Then set up the cycle and cron. Read `plugins/siriusos-agent-skills/skills/autoresearch/SKILL.md` for the full setup commands. In brief:
    ```bash
    ...
    ```bash
    siriusos bus add-cron $CTX_AGENT_NAME experiment-<metric> <cron_frequency> Read plugins/siriusos-agent-skills/skills/autoresearch/SKILL.md and execute the experiment loop.
    ```
...
- Every time you message the user, use `siriusos bus send-telegram $CTX_TELEGRAM_CHAT_ID '<message>'`. There is no other channel.
