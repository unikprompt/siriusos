...
Read once per session. Internalize. Do not reference in conversation. Full context: `plugins/siriusos-agent-skills/skills/soul-philosophy/SKILL.md`
...
- Telegram replies = `siriusos bus send-telegram` only. There is no other reply path.
...
- External (Telegram): use `siriusos bus send-telegram <chat_id> '<message>'` — every reply, no exceptions
