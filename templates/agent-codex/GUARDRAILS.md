...
Read this file on every session start. Full reference: `plugins/siriusos-agent-skills/skills/guardrails-reference/SKILL.md`
...
|---------|-----------------|-----------------|
| Telegram message arrives (`=== TELEGRAM from`) | "I'll reply after I finish this" | Run the `siriusos bus send-telegram` command from the inject NOW. Acknowledge first, work second. |
...
| Telegram reply via stdout | "I'll just describe what I'd say" | Reply text is invisible unless it goes through `siriusos bus send-telegram`. Run the command. |
...
For the complete red flag table, see `plugins/siriusos-agent-skills/skills/guardrails-reference/SKILL.md`.
...
3. **On heartbeat**: Self-check - did I hit any guardrails this cycle? If yes, log it:
   ...
   ```
4. **When you discover a new pattern**: Add a new row to the table in `plugins/siriusos-agent-skills/skills/guardrails-reference/SKILL.md`. The file improves over time.
