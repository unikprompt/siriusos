---
name: bus-reference
description: Complete siriusos bus CLI reference - all available commands with examples. Use when you need to look up a bus command, check syntax, or discover available tools.
---

# Bus Script Reference - COMPLETE TOOL INVENTORY

Every tool you have. Use them or the system cannot see your work.
All commands are available via `siriusos bus <command>`.
...
```bash
siriusos bus create-task "<title>" --desc "<description>" [--assignee <agent>] [--priority <p>] [--project <name>]
```
...
Example:
```bash
siriusos bus create-task "Write blog post" --desc "Draft a 500-word post on agent orchestration" --priority normal
```
...
```bash
siriusos bus update-task "<task_id>" <status>
```
...
Example:
```bash
siriusos bus update-task "task_abc123" in_progress
```
...
```bash
siriusos bus complete-task "<task_id>" --result "<what you produced>"
```
...
Example:
```bash
siriusos bus complete-task "task_abc123" --result "Deployed landing page to production. URL: https://site.com"
```
...
```bash
siriusos bus list-tasks [--status S] [--agent A] [--priority P] [--all-orgs]
```
...
```bash
siriusos bus claim-task <task_id> [--owner <agent>]
```
...
```bash
siriusos bus task-history <task_id>
```
...
```bash
siriusos bus check-deps <task_id>
```
...
```bash
siriusos bus save-output <task_id> <source_path> [--label "<short label>"]
```
...
Example:
```bash
siriusos bus save-output task_abc123 /tmp/audit-report.md --label "Audit report v1"
```
...
```bash
siriusos bus compact-tasks [--older-than <days>] [--dry-run]
```
...
```bash
siriusos bus send-message <target_agent> <priority> '<message_body>' [reply_to]
```
...
Example:
```bash
siriusos bus send-message <agent-name> high '{"action":"deploy","repo":"website","branch":"main"}'
```
...
Example:
```bash
siriusos bus ack-inbox "msg_xyz789"
```
...
```bash
siriusos bus log-event <category> <event_name> <severity> --meta '<json_payload>'
```
...
Examples:
```bash
siriusos bus log-event heartbeat agent_heartbeat info --meta '{"agent":"'$CTX_AGENT_NAME'"}'
siriusos bus log-event task task_completed info --meta '{"task_id":"task_abc123","summary":"Deployed site"}'
siriusos bus log-event error deploy_failed error --meta '{"repo":"website","error":"build timeout"}'
siriusos bus log-event action research_complete info --meta '{"topic":"competitor analysis","findings":3}'
```
...
```bash
siriusos bus update-heartbeat "<current_task_summary>"
```
...
Example:
```bash
siriusos bus update-heartbeat "WORKING ON: Implementing user auth for the dashboard"
```
...
```bash
siriusos bus recall-facts [--days N] [--limit M] [--format json|text]
```
...
```bash
siriusos bus create-approval "<title>" <category> "[context]"
```
...
Example:
```bash
siriusos bus create-approval "Send cold outreach to 50 leads" external-comms "Draft email attached in task_abc123. Target list: SaaS founders."
```
...
```bash
siriusos bus update-approval <approval_id> <approved|rejected> "[note]"
```

Example:
```bash
siriusos bus update-approval "appr_123" approved "User approved via Telegram"
```
...
```bash
siriusos bus send-telegram <chat_id> "<message>"
```
...
Example:
```bash
siriusos bus send-telegram "$CTX_TELEGRAM_CHAT_ID" "Task completed: Landing page deployed to production. URL: https://site.com"
```
...
```bash
siriusos bus edit-message <chat_id> <message_id> "<new_text>" [reply_markup_json]
```
...
```bash
siriusos bus answer-callback <callback_query_id> [toast_text]
```
...
```bash
siriusos bus list-agents [--org <org>] [--format json|text] [--status running|all]
```
...
```bash
siriusos bus list-skills [--format text|json]
```
...
```bash
siriusos bus check-stale-tasks [--all-orgs]
```
...
```bash
siriusos bus check-goal-staleness [--threshold DAYS] [--json]
```
...
```bash
siriusos bus archive-tasks [--dry-run] [--all-orgs]
```
...
```bash
siriusos bus notify-agent <agent_name> "<message>"
```
...
```bash
siriusos bus create-experiment <metric_name> "<hypothesis>" [--surface <path>] [--direction higher|lower] [--window <duration>] [--measurement <cmd>]
```
...
```bash
siriusos bus run-experiment <experiment_id> [changes_description]
```
...
```bash
siriusos bus evaluate-experiment <experiment_id> <measured_value> [--score <1-10>] [--justification "<text>"]
```
...
```bash
siriusos bus list-experiments [--agent <name>] [--status <status>] [--metric <name>] [--json]
```
...
```bash
siriusos bus gather-context [--agent <name>] [--metric <name>] [--format json|markdown]
```
...
```bash
siriusos bus manage-cycle <action> <agent>
```
...
```bash
siriusos bus self-restart --reason "why"
```
...
```bash
siriusos bus hard-restart --reason "why"
```
...
```bash
siriusos bus auto-commit [--dry-run]
```
...
`plugins/siriusos-agent-skills/skills/cron-management/SKILL.md`.
...
```bash
siriusos bus list-crons <agent> [--json]
```
...
```bash
siriusos bus test-cron-fire <agent> <name>
```
...
```bash
siriusos bus browse-catalog [--type skill|agent|org] [--tag <tag>] [--search <query>]
```
...
```bash
siriusos bus submit-community-item <item-name> <item-type> "<description>" [--dry-run]
```
...
- **Skill loaded**: `plugins/siriusos-agent-skills/skills/agent-browser/SKILL.md` — that skill instructs running `agent-browser skills get <name>` to fetch current per-version command syntax from the CLI itself
