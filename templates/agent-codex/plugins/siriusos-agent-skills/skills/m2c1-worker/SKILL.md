...
```bash
mkdir -p plugins/siriusos-agent-skills/skills/m2c1/artifact-templates

# Copy from the local SiriusOS framework install
for file in SKILL.md orchestration-workflow.md; do
  cp "${CTX_FRAMEWORK_ROOT}/templates/agent-codex/plugins/siriusos-agent-skills/skills/m2c1/$file" "./plugins/siriusos-agent-skills/skills/m2c1/$file"
done
```
...
```bash
mkdir -p "$PROJECT_DIR/plugins/siriusos-agent-skills/skills/comms"
cp "$CTX_FRAMEWORK_ROOT/templates/agent-codex/plugins/siriusos-agent-skills/skills/comms/SKILL.md" "$PROJECT_DIR/plugins/siriusos-agent-skills/skills/comms/SKILL.md" 2>/dev/null
```
...
## Your Role
You are the M2C1 orchestrator. Follow the 12-phase workflow in plugins/siriusos-agent-skills/skills/m2c1/orchestration-workflow.md.
...
2. Send a stuck alert to <your-agent-name>:
   ```
   siriusos bus send-message <your-agent-name> urgent 'STUCK ALERT: Detected repeated tool call loop. Tool: <tool-name>. Args: <args summary>. Repeated 5 times. Pausing for supervisor guidance.'
   ```
...
3. Send PLAN.md content to <your-agent-name>:
   ...
   siriusos bus send-message <your-agent-name> normal "PLAN READY FOR REVIEW
...
5. Once approved, read plugins/siriusos-agent-skills/skills/m2c1/orchestration-workflow.md and begin implementation.
...
**To approve:**
```bash
siriusos bus send-message <worker-name> normal 'PLAN_APPROVED. Proceed with implementation.'
```

**To request changes:**
```bash
siriusos bus send-message <worker-name> normal 'PLAN_REJECTED. Revise: <specific feedback>. Resend when updated.'
```
...
siriusos bus send-message <worker-name> normal 'Understood. Stop that approach. Instead: <alternative>. Continue from there.'

# If it is a transient error — tell worker to skip:
siriusos bus send-message <worker-name> normal 'Skip that step for now and continue to the next task. We will revisit.'

# If you need to inspect first:
cd $PROJECT_DIR && git log --oneline | head -5
# Then respond with a specific directive
siriusos bus send-message <worker-name> normal '<directive>'
```
...
# Copy the agent-browser SKILL.md into the worker's plugins/siriusos-agent-skills/skills/ so
# it is teachable to the worker session:
mkdir -p "$PROJECT_DIR/plugins/siriusos-agent-skills/skills/agent-browser"
cp "$CTX_FRAMEWORK_ROOT/templates/agent-codex/plugins/siriusos-agent-skills/skills/agent-browser/SKILL.md" \
   "$PROJECT_DIR/plugins/siriusos-agent-skills/skills/agent-browser/SKILL.md"

# 2. Worker can use agent-browser via Bash (no MCP restart required):
siriusos bus send-message <worker-name> normal \
  'agent-browser is available globally. Test by running: agent-browser open https://example.com && agent-browser get title && agent-browser close. Use snapshot-then-ref pattern for AI-driven flows. The plugins/siriusos-agent-skills/skills/agent-browser/SKILL.md was added — invoke `agent-browser skills get <name>` for current per-version command syntax.'
```
...
# is already in templates/agent-codex/plugins/siriusos-agent-skills/skills/agent-browser/SKILL.md and
...
```bash
siriusos bus add-cron <worker-name> task-poll 10m \
