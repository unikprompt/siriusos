...
siriusos bus kb-query "question" --org $CTX_ORG --agent $CTX_AGENT_NAME --scope private
...
**Add a one-shot reminder:** Use `siriusos bus add-cron $CTX_AGENT_NAME --name <name> --schedule <ISO> --prompt "<text>"` (one-time fire).
...
For ANY work that should survive restarts — heartbeats, daily reports, monitoring, experiment loops — use `siriusos bus add-cron`. That is the only way to guarantee the work runs forever.

| Need | Use |
|------|-----|
| Repeat for this session only | `/loop <interval> <prompt>` |
| Persist across restarts | `siriusos bus add-cron` |
| One-time future fire | `siriusos bus add-cron --schedule <ISO>` |
...
```bash
siriusos bus add-cron $CTX_AGENT_NAME daily-report "0 9 * * 1-5" Read .claude/skills/morning-review/SKILL.md and run the daily report.
```

**3. PR monitor every 4 hours, offset to avoid stampede:**
```bash
siriusos bus add-cron $CTX_AGENT_NAME pr-monitor "15 */4 * * *" Read .claude/skills/pr-monitor/SKILL.md and scan for new PRs.
```
