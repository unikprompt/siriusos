...
export const ecosystemCommand = new Command('ecosystem')
  ...
  .action(async (options: { instance: string; org?: string; output: string }) => {
    ...
    if (agents.length === 0) {
      console.log('No agents found. Add agents first: siriusos add-agent <name>');
      return;
    }
    ...
    // Without the second check, running `siriusos ecosystem` before
    ...
    // `siriusos ecosystem` after `npm install` to add it.
    ...
    // Now: `CTX_INSTANCE_ID=other pm2 restart siriusos-daemon` just works.
    //
    // BUG-016 fix: bumped max_restarts from 10 to 50. PM2's max_restarts
    // controls how many times PM2 itself restarts siriusos-daemon if it
    ...
    //
    // BUG-019 fix: emit a siriusos-dashboard PM2 entry alongside the daemon
    ...
    const content = `// AUTO-GENERATED by \`siriusos ecosystem\`. Do NOT edit by hand.
// Re-run \`siriusos ecosystem\` to regenerate.
//
...
// pm2 restart siriusos-daemon\` switches instances without regenerating.
module.exports = {
  apps: [
    {
