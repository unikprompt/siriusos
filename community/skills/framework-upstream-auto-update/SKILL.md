...
effort: low
description: "Daily upstream framework auto-update workflow. Fetches new siriusos commits, classifies by type and touched paths, auto-applies safe bug fixes, routes features/mixed to [ORCHESTRATOR] for approval, and verifies the apply with build + test before reporting."
...
## Owner
[AGENT_NAME] owns upstream framework health for the siriusos workspace. Bug fix application authority should be documented in your MEMORY.md or deployment config. Feature-level changes always route to [ORCHESTRATOR], who routes to the user for approval.

## Inputs
- Framework repo: the siriusos workspace root (the `CTX_ROOT` parent — the git repo, not the state dir)
...
When flagging: collect the commit SHAs, commit messages, and touched paths, and send them to [ORCHESTRATOR] via `siriusos bus send-message [ORCHESTRATOR] normal '<summary>'`. Do not apply.
...
```bash
siriusos bus send-message [ORCHESTRATOR] normal 'Framework upstream auto-update YYYY-MM-DD: applied N commits. Build + test green. Details: ...'
```
...
```bash
siriusos bus create-task "framework-upstream-check $(date +%Y-%m-%d)" --desc "Daily upstream check. Result: <applied N / flagged M / skipped K / noop>"
siriusos bus log-event action framework_updated info --meta '{"applied":N,"flagged":M,"skipped":K,"noop":BOOL}'
```
