...
6. **Crons are daemon-managed** — use `siriusos bus list-crons $CTX_AGENT_NAME` to see what's scheduled (no manual restore needed)
...
13. Send full online status — include what crons are scheduled (from `siriusos bus list-crons $CTX_AGENT_NAME`)
