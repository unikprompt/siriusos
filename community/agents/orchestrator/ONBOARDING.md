...
Check for existing crons first (`siriusos bus list-crons $CTX_AGENT_NAME` - avoid duplicates).

**Interval-based crons** - add via `siriusos bus add-cron` (persistent, survives restarts):
...
siriusos bus add-cron $CTX_AGENT_NAME approval-check 2h "Check for pending approvals: siriusos bus list-approvals --format json. Also check siriusos bus list-tasks --project human-tasks --status pending. For any pending approval or human task older than 1h, send user a Telegram reminder."
```

Do NOT use `/loop` for these — those crons are session-only and will not survive a restart.

**Time-anchored crons** - compute from context.json and add via `siriusos bus add-cron`:
...
Add the three time-anchored crons via `siriusos bus add-cron` using cron expressions (not intervals):
```bash
...
Then set up following `.claude/skills/autoresearch/SKILL.md` setup steps exactly. The cycle must be created with `siriusos bus manage-cycle create` including `--loop-interval`. Add the experiment cron immediately after:
```bash
