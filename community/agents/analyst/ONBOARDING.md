...
```bash
siriusos bus kb-ingest <path> --org $CTX_ORG --scope shared
```
...
**Set up the heartbeat cron via `siriusos bus add-cron`:**
...
Then verify with `siriusos bus list-crons $CTX_AGENT_NAME` — if a `heartbeat` entry already exists, skip adding a duplicate.

The default config also includes a `nightly-metrics` cron (24h) that runs `siriusos bus collect-metrics`. Verify it exists: `siriusos bus list-crons $CTX_AGENT_NAME`.
...
- Add via bus: `siriusos bus add-cron $CTX_AGENT_NAME <name> <interval> "<prompt>"`
...
For each enabled feature, register the cron via `siriusos bus add-cron`. The daemon persists it to `crons.json` and dispatches automatically — do not edit config.json directly.

**local_version_control** — register via `siriusos bus add-cron` (time-anchored, daemon-managed):
```bash
...
**upstream_sync** — register via `siriusos bus add-cron` (time-anchored, same hour, 2 minutes offset):
```bash
...
**catalog_browse** — register via `siriusos bus add-cron` (weekly, Sunday same hour):
```bash
...
Register via `siriusos bus add-cron` (time-anchored, daemon-managed):
```bash
