...
import type { AgentConfig, CtxEnv } from '../types.js';
...
export class AgentPTY {
  ...
  async spawn(mode: 'fresh' | 'continue', prompt: string): Promise<void> {
    ...
    // SiriusOS Single only supports the Anthropic Claude Code provider.
    // (Multi-provider support — openai/codex — lives in the full package.)
    const strategy: ProviderStrategy = anthropicStrategy;
