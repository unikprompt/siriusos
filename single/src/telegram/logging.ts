...
// SiriusOS Single: no bus/event analytics layer. The full package wires
// every inbound/outbound message into an Event stream consumed by the
// dashboard. Single just writes JSONL transcript files locally — that's
// sufficient context for the upgrade path (export tarball) and for
// the agent to read its own history.
import type { BusPaths, TelegramMessage } from '../types.js';
...
export function recordInboundTelegram(
  ...
  // (Single skips analytics event logging — JSONL transcript above is enough.)
}
